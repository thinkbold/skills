...
5. If a chart exists, accept only an active existing code with its exact chart name. If absent, require a nonempty free-form name and blank code. Only audit-authorized exact normalized active rules may auto-apply ledger-locally; fuzzy matches are suggestions requiring confirmation. Ask whether a choice affects selected rows only or a future exact rule.
...
- Never reuse prior external approval. Print the full operation disclosure and digest, pause for a current decision, then record that answer against the same digest. The user or agent may separately send only the disclosed/redacted data with an available third-party tool; the consent gate may then import its ledger-local, confirmed-account result. Process separate account/currency units separately.
